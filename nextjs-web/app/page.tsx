...
const LatestComments = dynamic(
	() => import("../src/components/LatestComments"),
	{ ssr: false }
);
				...
				<section>
					<Carousel autoplay className="rounded-lg overflow-hidden shadow">
						{featuredArticles.map((item) => (
							<div
								key={item.id}
								className="relative h-56 sm:h-72 flex items-center justify-center bg-gray-100"
							>
								<Image
									src={item.cover}
									alt={item.title}
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
						{latestArticles.map((item) => (
							...
							>
								...
								{/* 摘要 */}
								<div className="px-4 pt-2 pb-4 text-gray-500 text-sm min-h-[48px]">
									{item.summary}
								</div>
								{/* 数据区 */}
									...
									<div className="flex items-center gap-4">
										...
										<span className="flex items-center gap-1">
											<svg
												...
												viewBox="0 0 24 24"
											>
												<path d="M14 9l-1-1m0 0l-1 1m1-1v6m0 0l-1 1m1-1l1 1" />
											</svg>{" "}
											{item.likes || 88}
										</span>
										<span className="flex items-center gap-1">
											<svg
												...
												viewBox="0 0 24 24"
											>
												<path d="M17 8h2a2 2 0 012 2v10a2 2 0 01-2 2H5a2 2 0 01-2-2V10a2 2 0 012-2h2m10 0V6a4 4 0 00-8 0v2m8 0H7" />
											</svg>{" "}
											{item.comments || 12}
										</span>
									</div>
									<div className="text-right whitespace-nowrap">
										{item.date || "2024-06-19"}
									</div>
								</div>
							</Card>
						))}
			...
			<div className="flex-[3] flex flex-col gap-8 shrink-0 min-w-0">
				...
				<section>
					<Card className="text-center p-6">
						...
						{/* 名字 */}
						<div className="font-bold text-lg mt-3 mb-3">
							{siteOwner.name}
						</div>
						{/* 简介 */}

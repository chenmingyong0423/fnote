...
export const BaseCommentForm: React.FC<BaseCommentFormProps> = ({
  initialValues = {},
  onFinish,
  loading = false,
  submitText = "发表评论",
  ...
  return (
    <Form form={form} layout="vertical" onFinish={onFinish} className="mb-8">
      {preview ? (
        <div className="border rounded p-3 mb-2 bg-gray-50 dark:bg-[#232426] text-sm">
          <MarkdownPreview content={form.getFieldValue('content') || ''} />
        </div>
      ) : (
        <Form.Item name="content" label={contentLabel} rules={[{ required: true, message: '请输入内容' }]}> 
      ...
      <Form.Item noStyle shouldUpdate>
        {() => (
          <div className="w-full flex items-center gap-3 mb-4">
            ...
            <Form.Item name="website" className="mb-0 flex-1" rules={[{validator: (_, value) => {
      ...
      <div className="flex gap-2 items-center justify-center">
        ...
        {showPreview && onPreviewToggle && (
          <Button type="default" onClick={() => onPreviewToggle(!preview)}>
            {preview ? '关闭预览' : '预览'}
          </Button>
        )}
        {showClear && onClear && (
          <Button type="default" onClick={onClear}>清空</Button>
        )}
      </div>
    </Form>
  );

  ...
  >
    ...
    <div class="introduction flex flex-col items-center justify-center p-5">
      <span class="text-1.5em mb-2">{{
        homeStore.website_info.owner_name
      }}</span>
      <span class="text-gray-5 mb-2">{{
        homeStore.website_info.owner_profile
      }}</span>
    </div>
    <div
    ...
    >
      <div class="flex flex-col items-center justify-center w-33%">
        <span class="mb-1">{{ homeStore.website_count_stats.post_count }}</span>
        <span class="">文章</span>
      </div>
      <div
      ...
      >
        <span class="mb-1">{{
          homeStore.website_count_stats.category_count
        }}</span>
        <span class="">分类</span>
      </div>
      <div class="flex flex-col items-center justify-center w-33%">
        <span class="mb-1">{{ homeStore.website_count_stats.tag_count }}</span>
        <span class="">标签</span>
      </div>
    </div>
    <div
    ...
    >
      <div class="flex flex-col items-center justify-center w-33%">
        <span class="mb-1">{{
          homeStore.website_count_stats.comment_count
        }}</span>
        <span class="">评论</span>
      </div>
      <div
        class="flex flex-col items-center justify-center w-33% border-x-1 border-x-gray-2 border-x-solid"
      >
        <span class="mb-1">{{ homeStore.website_count_stats.like_count }}</span>
        <span class="">点赞</span>
      </div>
      <div class="flex flex-col items-center justify-center w-33%">
        <span class="mb-1">{{
          homeStore.website_count_stats.website_view_count
        }}</span>
        <span class="">浏览量</span>
      </div>
    </div>
    ...
    >
      <template
        v-for="(icon, index) in homeStore.social_info_list"
        :key="index"
      >
        <a
          v-if="icon.is_link"
          :class="get(icon.css_class)"
          class="custom_icon text-6"

    ...
    return friends.map((item) => (
      <Col xs={24} sm={12} lg={8} key={item.url}>
        <Card
          ...
          extra={<a href={item.url} target="_blank" rel="noreferrer" className="dark:text-blue-400">访问</a>}
        >
          <Space align="start" direction="horizontal" size="middle">
            ...
            <Paragraph className="!mb-0 dark:!text-gray-300">{item.description || "这个站长很神秘，还没有写描述。"}</Paragraph>
          </Space>
        </Card>
      </Col>
    ));
  ...
  return (
    <div className="w-full max-w-7xl mx-auto md:px-4 py-6 md:py-8 flex flex-col gap-8 md:gap-12 bg-white p-4 md:p-6 rounded-xl shadow-sm dark:bg-[#141414] dark:text-gray-300 dark:border dark:border-[#303030]">
      ...
      <section className={sectionClassName}>
        <Title level={2} className={titleClassName}>友链</Title>
        <Paragraph type="secondary" className={secondaryTextClassName}>欢迎互换友链，先看看申请须知和注意事项。</Paragraph>
        {summary ? <MarkdownPreview content={summary} /> : <Empty description="暂无申请须知" className={emptyClassName} />}
      </section>

      <section className={sectionClassName}>
        <Title level={3} className={titleClassName}>友链列表</Title>
        {friends && friends.length > 0 ? (
          <Row gutter={[16, 16]} className="mt-4">
            {friendCards}
          </Row>
        ) : (
          <Empty description="暂无友链" className={`my-8 ${emptyClassName}`} />
        )}
      </section>

      <section className={sectionClassName}>
        <Title level={3} className={titleClassName}>申请友链</Title>
        <Paragraph type="secondary" className={`mb-4 ${secondaryTextClassName}`}>
          请填写以下信息，我们会尽快审核。
        </Paragraph>
        <Form form={form} layout="vertical" onFinish={onFinish} initialValues={{ url: "https://" }} className={formClassName}>
          ...
          <Form.Item label="Logo" name="logo" rules={[{ required: true, message: "请填写 logo 链接" }]}>
            <Input placeholder="https://example.com/logo.png" className={inputClassName} />
          </Form.Item>
          <Form.Item
            label="描述"
            name="description"
          ...
          >
            <Input.TextArea placeholder="一句话介绍你的站点" rows={3} showCount maxLength={80} className={inputClassName} />
          </Form.Item>
          <Form.Item
            label="邮箱"
            name="email"
          ...
          >
            <Input placeholder="用于联系，审核结果会通知" className={inputClassName} />
          </Form.Item>
          <Form.Item>
